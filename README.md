...
- **Visitor date picker isn't restricted to Sunday–Thursday.** The "Select date" bottom sheet is visually reused for both room bookings and visitor registration in Figma, but the PDF's business-day restriction is only listed under Meeting Room Booking, not Visitor Management. The restriction is therefore only enforced (and only shown as a caption) for room bookings.
- **No real room photos.** Figma's room-details frame includes room photography; no exported image assets were available for those specific photos, so room imagery is a placeholder icon tile instead of invented images. The onboarding illustrations, by contrast, *are* the actual artwork exported from Figma (`assets/images/`).
